...
const { Student, Payment, Staff, Settings, Audit, User, LevyType, StudentLevy, LevyPayment, SchoolDocument } = require('./models_index');
...
// ── Documents ─────────────────────────────────────────────
router.get('/documents', requireSchoolAuth, async (req, res) => {
  try {
    const docs = await SchoolDocument.find({ schoolId: sid(req) }).sort({ createdAt: -1 });
    res.json(docs);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

router.post('/documents', requireSchoolAuth, async (req, res) => {
  try {
    const count = await SchoolDocument.countDocuments({ schoolId: sid(req) });
    const id = 'DOC' + String(count + 1).padStart(4, '0');
    ...
    await Audit.create({ schoolId: sid(req), time: new Date().toISOString(), user: req.user.displayName, action: 'DOCUMENT_UPLOADED', detail: req.body.title });
    res.status(201).json(doc);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

router.delete('/documents/:id', requireSchoolAuth, async (req, res) => {
  try {
    await SchoolDocument.findOneAndDelete({ schoolId: sid(req), id: req.params.id });
    res.json({ success: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// Public endpoint — for parent portal (no auth, just needs matching schoolId via slug)
router.get('/documents/student/:studentId', async (req, res) => {
  try {
    ...
    }).select('-fileData -stamp').sort({ createdAt: -1 });
    res.json(docs);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
